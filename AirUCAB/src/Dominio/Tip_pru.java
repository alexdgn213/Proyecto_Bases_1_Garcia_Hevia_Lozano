...
public class Tip_pru {
    ...
    public void agregarADB(ConectorDB conector){
        try{
            String stm = "INSERT INTO Tip_pru(fk__tip_codigo,fk_pru_codigo) VALUES(?,?)";
    ...
    public static void llenarTabla(ConectorDB conector, JTable jTable){
        ResultSet rs =obtenerResultSet(conector,"SELECT tip_pru_codigo as Codigo,f_tip_codigo,fk_pru_codigo as Codigo_Prueba FROM Tip_pru");
    ...
    public static Tip_pru buscarPorCodigo(ConectorDB conector, int codigo){
        Tip_pru tp = null;
        try {
            PreparedStatement pst = conector.conexion.prepareStatement("SELECT tip_pru_codigo, fk_tip_pru,fk_pru_codigo FROM tip_pru WHERE tip_pru_codigo=?");
            pst.setInt(1, codigo);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
